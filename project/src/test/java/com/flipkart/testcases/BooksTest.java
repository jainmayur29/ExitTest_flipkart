...
public class BooksTest extends basePage {
	@Test
	public void literaturebooktest() throws InterruptedException {
		BooksPage bp = PageFactory.initElements(driver, BooksPage.class);
		test.log(LogStatus.INFO, "User hovers on the Books Button");
		test.log(LogStatus.INFO, "User clicks on the Literature & Fiction Button");
		bp.book();
		test.log(LogStatus.INFO, "User can see the Literature & Fiction Books");
		WebElement element = BooksPage.book_result;
		String a = element.getText();
		Assert.assertEquals(ReadingTestData.getProperty("books_verify"), a);
	}
}
